 ...
 */
 ...
 // this plugin is edited by sri vishnu totakura(vishnu@devbrother.com) in order to make it compatible with filmmo.
...
(function($){
    ...
    TimeAutocomplete.prototype = {
        ...
        _setValueAsTime: function(){
            ...
            // edited by sri vishnu totakura for fixing the issue when the time is already set. Use it.

            if($.trim(this.el.val()) != ''){
              // if value is not empty then leave it as is.

            }else if($.trim(this.el.val()) == '' && this.options.value){
                this.setTime(this.options.value);
            } else {
                // var time = this._getCurrentTimeAsValue();
                var time = this._callFormatterMethod('_createStringFromFormat', [this.options.empty])
                this.el.val(time);
                this._attacheUsableTimeData();
            }
